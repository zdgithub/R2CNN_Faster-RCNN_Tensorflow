...
class DetectionNetwork(object):
    ...
    def postprocess_fastrcnn_r_my(self, rois, bbox_ppred, scores, img_shape):
        '''
        ...
        with tf.name_scope('postprocess_fastrcnn_r'):
            ...
            for i in range(1, cfgs.CLASS_NUM+1):
                ...
                allclasses_boxes.append(tmp_decoded_boxes)
                allclasses_scores.append(tmp_score)
                categories.append(tf.ones_like(tmp_score) * i)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch):
        ...
        if not self.is_training:
            final_boxes_h, final_scores_h, final_category_h = self.postprocess_fastrcnn_h(rois=rois,
                                                                                          ...
                                                                                          scores=cls_prob_h,
                                                                                          img_shape=img_shape)
            all_boxes_r, all_scores_r, all_category_r = self.postprocess_fastrcnn_r_my(rois=rois,
